...
\author{
Sayam Parejiya \hspace*{1.5cm} Harsh Manek \hspace*{1.5cm} Dr. Archna N. Vyas \\
\normalsize Department of Information Technology \hspace*{0.5cm} Department of Information Technology \hspace*{0.5cm} Department of Information Technology \\
\normalsize Dharmsinh Desai University \hspace*{1.3cm} Dharmsinh Desai University \hspace*{1.3cm} Dharmsinh Desai University \\
\normalsize Nadiad, Gujarat, India \hspace*{1.8cm} Nadiad, Gujarat, India \hspace*{1.8cm} Nadiad, Gujarat, India \\
\normalsize sayamparejiya@gmail.com \hspace*{1.2cm} harshmanek786@gmail.com \hspace*{1.5cm} anvyas@ddu.ac.in
}
...
The proposed system follows a modular four-stage pipeline: Data Input (stock prices, sentiment scores, macroeconomic indicators) $\rightarrow$ Preprocessing (data cleaning, normalization, feature engineering) $\rightarrow$ Model Inference (Random Forest classification) $\rightarrow$ Output (predictions, feature importance, API endpoints, dashboard visualizations).
...
\subsection{Data Analysis and Feature Distributions}

\subsubsection{Market Correlations}

Analysis of TCS stock price versus USD/INR exchange rate (Figure \ref{fig:tcs_usd}) reveals significant correlation during major market events, validating our multimodal feature approach. The dual-axis representation clearly demonstrates how currency fluctuations influence equity valuations.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{tcs_usd_correlation.png}
\caption{TCS Stock Price vs. USD/INR Exchange Rate showing strong market correlation dynamics}
\label{fig:tcs_usd}
\end{figure}

\subsubsection{Sentiment-Price Dynamics}

Figure \ref{fig:sentiment_news} illustrates the relationship between LEMONTREE financial news sentiment and stock price movements over time. Daily sentiment scores from news portals often precede price movements by 1-2 days, demonstrating sentiment's predictive power as a leading indicator.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{lemontree_sentiment.png}
\caption{LEMONTREE - Price and News Sentiment (Dual Axis) showing sentiment-price lead-lag relationship}
\label{fig:sentiment_news}
\end{figure}

\subsubsection{Feature Distribution Analysis}

Figure \ref{fig:distributions} presents the distribution of key features across the dataset. Close prices exhibit right-skewed distribution due to stock price growth over time, while sentiment scores show concentration near neutral (zero) with occasional extreme values reflecting major news events.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{feature_distributions.png}
\caption{Feature Distributions for Close Price, Sentiment Score, USD/INR Rate, and Interest Rate}
\label{fig:distributions}
\end{figure}

\subsubsection{Multi-Factor Time Series Analysis}

Figure \ref{fig:multifactor} shows the integrated dynamics of price, sentiment, USD/INR rate, and RBI Repo Rate over the analysis period. This visualization validates that multiple macroeconomic and sentiment factors move in synchronized patterns, supporting our multimodal approach.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{tcs_multifactor.png}
\caption{TCS: Price, Sentiment, USD/INR Rate, and Repo Rate - Multi-factor dynamics}
\label{fig:multifactor}
\end{figure}

\subsection{Feature Correlation Analysis}

Figure \ref{fig:correlation} presents the feature correlation matrix, revealing key relationships between variables. Notably, USD/INR rate and interest rate show moderate positive correlation (0.55), while sentiment maintains low correlation with price movements (0.02), indicating sentiment captures orthogonal information valuable for prediction.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{correlation_matrix.png}
\caption{Feature Correlation Matrix showing relationships between sentiment, exchange rate, interest rate, and close price}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Stock} & \textbf{Accy.} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} \\
\midrule
TCS & 67.0\% & 0.68 & 0.66 & 0.67 \\
HDFCBANK & 66.0\% & 0.65 & 0.66 & 0.65 \\
BAJFINANCE & 65.0\% & 0.64 & 0.63 & 0.64 \\
\midrule
\textbf{Avg.} & \textbf{66.5\%} & \textbf{0.66} & \textbf{0.65} & \textbf{0.65} \\
\bottomrule
\end{tabular}
...
Figure \ref{fig:confusion} visualizes the confusion matrix on test data, showing the model's ability to identify both up and down movements. The heatmap reveals strong diagonal dominance with 2586 true negatives and 821 true positives, indicating robust classification performance.

\begin{figure}[h]
\centering
\includegraphics[width=0.70\linewidth]{confusion_matrix.png}
\caption{Confusion Matrix heatmap showing prediction accuracy and error distribution across test set}
\label{fig:confusion}
\end{figure}

\subsection{Feature Importance Analysis}

Figure \ref{fig:importance} reveals the relative importance of features in the Random Forest classifier. Close price (0.62) dominates, followed by sentiment polarity (0.10) and USD/INR rate (0.16), validating the importance of multimodal data integration.

\begin{figure}[h]
\centering
\includegraphics[width=0.90\linewidth]{feature_importance.png}
\caption{Random Forest Feature Importance showing relative contribution of each feature to predictions}
\label{fig:importance}
\end{figure}

Figure \ref{fig:importance_events} extends this analysis by including event-based features (days to next event, days since last event, event window indicator). This comprehensive ranking demonstrates that event proximity contributes meaningfully to model predictions, especially for scheduled announcements.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{feature_importance_events.png}
\caption{Feature Importances Including Events - Extended analysis with event-based features}
\label{fig:importance_events}
\end{figure}
...
Figure \ref{fig:strategy} compares the cumulative returns of our predictive strategy against a buy-and-hold baseline. The strategy exhibits superior risk-adjusted returns, particularly during volatile market periods, demonstrating practical trading applicability.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{strategy_performance.png}
\caption{Strategy vs Buy/Hold Cumulative Return comparing predictive strategy performance against baseline}
\label{fig:strategy}
\end{figure}
...
\begin{itemize}
\item \texttt{/predict/\{stock\_symbol\}} - Real-time prediction
...
\item \textbf{Extreme Events:} Market dislocations or circuit breakers not well-captured without explicit anomaly detection.
\end{enumerate}
...
\item \textbf{Alternative Data:} Integrate satellite imagery, credit card transactions, or blockchain activity as alternative data sources.

\item \textbf{Adaptive Learning:} Develop online learning mechanisms where Random Forest continuously updates with new data.

\item \textbf{Explainability:} Implement SHAP values for instance-level prediction explanations.
...
We extend sincere gratitude to Dr. Archna N. Vyas for her invaluable guidance and mentorship throughout this research. We thank the Department of Information Technology at Dharmsinh Desai University for providing resources and support.
